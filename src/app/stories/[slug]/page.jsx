...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API}/api/stories/?wo-paginate=1`, {
      // REQUIRED headers to generate static params! 
      method: 'GET',
      // revalidate: 0,
      cache: 'no-store',
      headers: {
...
export async function generateMetadata({ params }) {
  const { slug } = params;
  const data = await fetch(`${process.env.NEXT_PUBLIC_API}/api/stories?slug=${slug}`, {
    cache: 'no-store'
  }).then((res) => res.json());
...
export default async function Page({ params }) {
  ...
  // Problem with getItem ID when generating run build...
  const dataContent = await fetch(`${process.env.NEXT_PUBLIC_API}/api/stories?w-content=1&id=${getItemId}&wo-paginate=1`,
    ...
    <section className='mb-20'>
        ...
        <div className='flex w-full items-end justify-between mx-auto flex-col md:flex-row  mt-[4rem]'>
          ...
          <div className='flex flex-1 flex-col w-full md:w-50'>
            <div className='m-7 md:m-14'>
              ...
              <p className='text-lg lg:text-xl font-poppins'>{item?.sub_title}</p>
            </div>
          </div>
      ...
      <div className='max-w-[1480px] mx-auto mt-10 md:mt-14 mb-20 px-6 flex flex-col lg:flex-row justify-center lg:space-x-20'>
        ...
        <div className='flex-1 w-full lg:max-w-[700px]'>
          ...
          {dataContentsContent?.map((item, index) =>
          (
              ...
              <div className='content'>
                ...
                {item.type === "quote" && (
                  <blockquote className='mb-10'>
                    <div className='w-auto max-w-[420px]  mb-5 relative'>
                      <Image src='/images/qout-left.svg' width='30' height='28' className='mb-2' alt='Quote' />
                      <h2 className='qoute-text' dangerouslySetInnerHTML={{ __html: item?.content }}></h2>
                      <Image src='/images/qout-right.svg' width='30' height='28' className='absolute bottom-[-.5rem] right-[-2rem]' alt='Quote' />
                    </div>
                ...
                {item?.additional_content === "two-columns" &&
                  <div className='grid grid-cols-2 gap-5 mb-10 two-columns' >
                    ...
                    {item.content_items.map((item, index) =>
                      <figure key={item?.id}>
                ...
                {item.additional_content === "normal" && (
        ...
        <div className='flex-none lg:w-[200px] pt-3'>
          ...
          <div className='flex flex-col space-y-6 divide-y-2'>
            ...
            {item?.related_name &&
              item?.related_name?.map((item) => {

                return (
                  <div className='pt-5' key={item}>
                    <h5 className='text-[#4E4E4E] font-opensans leading-5 font-bold mb-3 text-[16px] line-clamp-3' >{item?.name}</h5>
                    <p className='font-poppins text-[#797979] text-[14px] mb-1 capitalize tracking-[1px]'>
                    ...
                    <Link class='relative group' href={`/stories/${item.slug}`} title='Read'>Read
                      <span class="absolute -bottom-2 w-[100%] left-0 -right-2 h-[12%] origin-left scale-x-0 rounded-full bg-[#E8412C] transition-transform duration-300 ease-out group-hover:scale-x-100 "></span></Link>
                  </div>
                )
              }
              )
        ...
        <div className='grid sm:grod-cols-2 md:grid-cols-3 sm:gap-7 md:gap-14 space-y-12 md:space-y-0'>
          ...
          <div className='w-full space-y-4'>
            ...
            <div className='space-y-3'>
              ...
              <Link className='group relative' href='/' title='title'>
                Read
                <span class="absolute -bottom-2 w-[100%] left-0 -right-2 h-[12%] origin-left scale-x-0 rounded-full bg-[#E8412C] transition-transform duration-300 ease-out group-hover:scale-x-100 "></span>
              </Link>
            </div>
          </div>
